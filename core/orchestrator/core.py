...
from extensions.codex.daemon import CodexDaemon, codex_daemon as _global_codex_daemon
...
_NOTIFICATION_QUEUES: dict[
    SessionKey, asyncio.Queue[tuple[str, dict[str, Any]]]
] = {}
...
class Orchestrator:
    ...
    def __init__(
        ...
        self._message_bus = MessageBus(
            lock_pool=_global_session_manager.lock_pool,
    ...
    async def _on_codex_notification(
    ...
    async def _on_codex_server_request(
        self, method: str, params: dict[str, Any]
    ) -> dict[str, Any] | None:
        if method in (
            ...
            if self._approval_ui_sender is not None:
                try:
    ...
    async def handle_message_streaming(
        ...
        *,
        ...
        """
        routed = self._route_message(key, text)
        return await self._execute_routed_streaming(
            key, routed, db, user_id, callbacks
        )
        ...
        if cmd_name is not None and handler is not None:
            ...
            if cmd_name == "shell":
                return _RoutedAction(
                    action="shell_command", payload={"command": rest}
                )
            return _RoutedAction(
        ...
        if directive is not None:
            if directive.name == "codex":
                return _RoutedAction(
                    action="codex_prompt",
                    payload={
                        "prompt": directive.rest or directive.target or ""
                    },
                )
            if directive.name == "shell":
                return _RoutedAction(
                    ...
                    payload={
                        "command": directive.rest or directive.target or ""
                    },
                )
            if directive.name == "ai":
                return _RoutedAction(
                    action="ai_chat", payload={"prompt": directive.rest}
                )

        # 3. NamedSession → Normal Flow
        # The handler decides whether this key belongs to a named Codex
        # session before calling Orchestrator.  Default to normal AI flow.
    ...
    def _route_codex_command(self, key: SessionKey, rest: str) -> _RoutedAction:
        ...
        if stripped.startswith("cd "):
            return _RoutedAction(
                action="codex_cd", payload={"path": rest[3:].strip()}
            )
        if stripped == "pwd":
        ...
        if prefix == "shell":
            return _RoutedAction(
                action="shell_command", payload={"command": instr_rest}
            )
        if prefix == "file":
            return _RoutedAction(
                action="codex_file",
        ...
        # Normal prompt
        return _RoutedAction(
            action="codex_prompt", payload={"prompt": rest}
        )
    ...
    async def _execute_routed(
        ...
        if action == "codex_prompt":
            return await self._run_codex_prompt(
                key, payload.get("prompt", ""), db, user_id
            )
        if action == "shell_command":
            result = await self._run_shell_command(
                payload.get("command", ""), session_id=key.to_string()
            )
            return self._format_shell_result(result)
        if action == "ai_chat":
            return await self._run_ai_chat(
                key, payload.get("prompt", ""), db, user_id
            )
        if action == "codex_new":
        ...
        if action == "codex_status":
            ...
            if resume:
                lines.append(
                    f"_Resumed session (last active {resume.get('resumed_at', 'unknown')})_"
                )
            return "\n".join(lines)
        if action == "codex_cd":
            info = await self.codex_set_cwd(
                key, db, payload.get("path", "")
            )
            return (
                f"Working directory changed.\n"
                f"New thread: `{info['thread_id']}`\n"
                f"CWD: `{info['cwd']}`"
            )
        if action == "codex_pwd":
        ...
        if action == "codex_file":
            entries = await self.codex_list_directory(
                payload.get("path", ".")
            )
    ...
    async def _execute_routed_streaming(
        ...
        if action == "codex_prompt":
            return await self._stream_codex_prompt(
                key, payload.get("prompt", ""), db, user_id, callbacks
            )
        if action == "shell_command":
            result = await self._run_shell_command(
                payload.get("command", ""), session_id=key.to_string()
            ...
            return text
        if action == "ai_chat":
            return await self._stream_ai_chat(
                key, payload.get("prompt", ""), db, user_id, callbacks
            )
    ...
    async def codex_new_session(
        self, key: SessionKey, db: AsyncSession, user_id: int
    ) -> dict[str, Any]:
        if self._session_manager is None:
            raise RuntimeError("Codex session manager is not available")
        session = await self._session_manager.new_session(
            db=db, session_key=key, user_id=user_id
        )
        return {
    ...
    async def codex_set_cwd(
        self, key: SessionKey, db: AsyncSession, path: str
    ) -> dict[str, Any]:
        if self._session_manager is None:
            raise RuntimeError("Codex session manager is not available")
        await self._session_manager.set_cwd(
            db=db, session_key=key, cwd=path
        )
        ...
        session = await self._session_manager.get_or_create_session(
            db=db, session_key=key, user_id=0, cwd=path
        )
        return {
            "thread_id": session.thread_id,
    ...
    async def codex_get_pwd(
        self, key: SessionKey, db: AsyncSession
    ) -> str | None:
    ...
    async def codex_list_threads(
        ...
        if self._session_manager is None:
            return []
        return await self._session_manager.list_threads(
            db=db, session_key=key
        )

    async def codex_archive(
        self, key: SessionKey, db: AsyncSession
    ) -> bool:
        if self._session_manager is None:
            return False
        return await self._session_manager.archive_thread(
            db=db, session_key=key
        )

    async def codex_switch(
        self, key: SessionKey, db: AsyncSession, conversation_id: str
    ) -> bool:
        if self._session_manager is None:
            return False
        if not conversation_id.isdigit():
            return False
        return await self._session_manager.activate_thread(
            db=db, session_key=key, conversation_id=int(conversation_id)
        )
        ...
        try:
            while True:
                try:
                    ...
                    await _maybe_await(
                        callbacks.on_error, "Turn timed out"
                    )
                    break
                ...
                if method == "turn/completed":
                    ...
                    await _maybe_await(
                        callbacks.on_turn_completed, turn, final_text
                    )
                    ...
                    if delta:
                        ...
                        await _maybe_await(
                            callbacks.on_text_delta, delta, accumulated
                        )
                    ...
                    self._approval_handler.cache_item(
                    ...
                    item_type = params.get("item", {}).get("type", "")
                    if item_type == "commandExecution":
                        exit_code = params.get("item", {}).get(
                            "exitCode", ""
                        )
                        output = params.get("item", {}).get(
                            "aggregatedOutput", ""
                        )
                        block = f"\nExit code: {exit_code}"
                        if output:
    ...
    async def _run_shell_command(
        ...
        return await self._shell_provider.execute(
    ...
    def _register_default_commands(self) -> None:
        self.commands.register(
            "codex", self._noop_cmd, description="Codex bridge commands"
        )
        self.commands.register(
            "shell", self._noop_cmd, description="Execute shell commands"
        )
