...
async def test_cmd_shell_delegates_to_shell_ui(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    orchestrator = SimpleNamespace()
    context = SimpleNamespace()
    handle_shell_command = AsyncMock()
    monkeypatch.setattr(codex.shell_ui, "handle_shell_command", handle_shell_command)

    await codex.cmd_shell(message, orchestrator, context)

    handle_shell_command.assert_awaited_once()
    args = handle_shell_command.await_args.args
    kwargs = handle_shell_command.await_args.kwargs
    assert args[:3] == (message, orchestrator, context)
    assert kwargs["ensure_orchestrator"] is codex._ensure_global_orch
    assert kwargs["execute_shell"] is codex._execute_shell_telegram
...
async def test_shell_callbacks_delegate_to_shell_ui(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    handle_shell_ai_callback = AsyncMock()
    handle_shell_approve_callback = AsyncMock()
    monkeypatch.setattr(codex.shell_ui, "handle_shell_ai_callback", handle_shell_ai_callback)
    monkeypatch.setattr(codex.shell_ui, "handle_shell_approve_callback", handle_shell_approve_callback)

    await codex.handle_shell_ai_callback(callback_query, orchestrator)
    await codex.handle_shell_approve_callback(callback_query, orchestrator)

    handle_shell_ai_callback.assert_awaited_once_with(
        callback_query,
        orchestrator,
        execute_shell=codex._execute_shell_telegram,
    )
    handle_shell_approve_callback.assert_awaited_once_with(
        callback_query,
        orchestrator,
        execute_shell=codex._execute_shell_telegram,
    )
