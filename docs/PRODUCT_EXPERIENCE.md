...
用户想在 Telegram 里控制 Codex 时，最自然的入口是在 forum group 里创建一个 Codex topic。用户可以发送 `/codex new`，也可以在 Codex 菜单里点 `新建 Codex Topic`。Bot 会创建一个新的 forum topic，名字可以包含 `Codex`、项目名、短 thread 标识或当前工作目录提示。创建完成后，Bot 会把这个 topic 和一个新的 Codex 会话绑定。
...
用户也可以在 Bot 私聊里发送 `/codex <prompt>`。这个模式适合轻量使用，比如让 Codex 分析一个小片段、检查当前仓库状态、执行一个短命令。私聊模式也要保留 Codex thread，但它不如 forum topic 适合多个项目并行。
