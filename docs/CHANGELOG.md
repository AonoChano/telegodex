...
- Restored the in-place Telegram polling reconnect status as the default display, while keeping `TELEGODEX_POLLING_INLINE_STATUS=0` for diagnostic line-by-line logs.
...
- Updated the live reconnect status so backoff shows `retry in <duration>`, active health checks show `probing`, elapsed time uses rolled-up integer durations, and full error details wrap in a controlled terminal block instead of being truncated.
